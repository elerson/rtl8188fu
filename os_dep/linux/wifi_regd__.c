						     ...
						     request))
{
	...
	regd = rtw_regdomain_select(NULL);
	wiphy_apply_custom_regulatory(wiphy, regd);
	_rtl_reg_apply_radar_flags(wiphy);
	_rtl_reg_apply_world_flags(wiphy, NL80211_REGDOM_SET_BY_DRIVER, NULL);
	return 0;
}
...
void rtl_reg_notifier(struct wiphy *wiphy, struct regulatory_request *request)
{
	...
	_rtl_reg_notifier_apply(wiphy, request, reg);
}
...
int rtl_regd_init(struct wiphy *wiphy)
{

	_rtl_regd_init_wiphy(NULL, wiphy, rtl_reg_notifier);
...
void rtw_reg_notify_by_driver(struct wiphy *wiphy)
{
	if (wiphy) {
		...
		rtl_reg_notifier(wiphy,  &request);
	}
}
